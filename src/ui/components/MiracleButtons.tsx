...
type AbilityButtonStyle = CSSProperties & {
...
export function MiracleButtons() {
  ...
  const previousSkills = useRef<readonly DivineSkillId[]>(
    unlockedSkills,
  );
  const [flashingSkill, setFlashingSkill] =
  ...
  useEffect(() => {
    const acquired = unlockedSkills.find(
      (skill) => !previousSkills.current.includes(skill),
    );
  ...
  useEffect(() => {
    const activateByKey = (event: KeyboardEvent) => {
      ...
      const miracleIndex = MIRACLE_KEYS.findIndex(
      ...
      if (miracleIndex >= 0) {
        ...
        if (miracle !== undefined && miracleEnabled(miracle)) {
          ...
          selectMiracle(
            selectedMiracle === miracle ? null : miracle,
          );
        }
        return;
      }
      const skillIndex = SKILL_KEYS.findIndex(
        (candidate) => candidate === key,
      );
  ...
  return (
    <section className="miracle-panel" aria-label="Divine abilities">
      ...
      <div className="miracle-button-list">
        {MIRACLE_ORDER.map((type, index) => {
          ...
          const style: AbilityButtonStyle = {
          ...
          return (
            <button
              ...
              key={type}
              onClick={() =>
                selectMiracle(selected ? null : type)
              }
              style={style}
              type="button"
            >
              ...
              {cooldown > 0 ? (
                <span className="miracle-cooldown">
                  {(
                    cooldown / BALANCE_CONFIG.TICKS_PER_SECOND
                  ).toFixed(1)}
                  s
                </span>
              ) : null}
        ...
        {unlockedSkills.map((skill, index) => {
          ...
          return (
            <button
              ...
              className={
                flashingSkill === skill
                  ? "miracle-button miracle-button--new"
                  : "miracle-button"
              }
              disabled={!enabled}
              key={skill}
