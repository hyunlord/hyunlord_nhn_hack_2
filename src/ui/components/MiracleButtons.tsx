...
import { skillDescription, skillName } from "../../content/locale/display";
...
const MIRACLE_LABEL_KEYS: Readonly<Record<MiracleType, LocaleKey>> = {
  blessing: "miracle.blessing",
  curse: "miracle.curse",
  lightning: "miracle.lightning",
};

type AbilityButtonStyle = CSSProperties & { readonly "--miracle-color": string };

export function MiracleButtons() {
  const { dispatch, selectedMiracle, selectedSkill, selectMiracle, selectSkill, state } = useGameStore();
  const { t } = useLocale();
  ...
  const miracleEnabled = (type: (typeof MIRACLE_ORDER)[number]) => activePhase && canCast(type, divinePower, miracleCooldowns[type], modifiers);
  const skillEnabled = (skill: DivineSkillId) => activePhase && canCastSkill(skill, unlockedSkills, divinePower, skillCooldowns[skill], modifiers.divineCostMultiplier);

  function activateSkill(skill: DivineSkillId): void {
    if (!skillEnabled(skill)) return;
    const definition = DIVINE_SKILL_DEFINITIONS[skill];
    if (!definition.targeted) {
      dispatch({ type: "castSkill", skill, x: BALANCE_CONFIG.WORLD_WIDTH / 2, y: BALANCE_CONFIG.WORLD_HEIGHT / 2 });
      return;
    }
    ...
    <section className="miracle-panel run-hud-bottom-center" aria-label={t("abilities.label")}>
      ...
      <div className="miracle-button-list">
        {MIRACLE_ORDER.map((type, index) => {
          ...
          return <button aria-pressed={selected} className="miracle-button" disabled={!enabled} key={type} onClick={() => selectMiracle(selected ? null : type)} style={style} type="button"><kbd>{MIRACLE_KEYS[index]}</kbd><span>{t(MIRACLE_LABEL_KEYS[type])}</span><span className="miracle-cost">{t("abilities.cost", { cost: (definition.cost * modifiers.divineCostMultiplier).toFixed(0) })}</span>{cooldown > 0 ? <span className="miracle-cooldown">{t("abilities.cooldown", { seconds: (cooldown / BALANCE_CONFIG.TICKS_PER_SECOND).toFixed(1) })}</span> : null}</button>;
        })}
        {unlockedSkills.map((skill, index) => {
          ...
          const style: AbilityButtonStyle = { "--miracle-color": definition.color };
          return <button aria-pressed={selected} className={flashingSkill === skill ? "miracle-button miracle-button--new" : "miracle-button"} disabled={!enabled} key={skill} onClick={() => activateSkill(skill)} style={style} title={skillDescription(t, skill)} type="button"><kbd>{SKILL_KEYS[index] ?? "-"}</kbd><span>{skillName(t, skill)}</span><span className="miracle-cost">{t("abilities.cost", { cost: (definition.cost * modifiers.divineCostMultiplier).toFixed(0) })}</span>{cooldown > 0 ? <span className="miracle-cooldown">{t("abilities.cooldown", { seconds: (cooldown / BALANCE_CONFIG.TICKS_PER_SECOND).toFixed(1) })}</span> : null}</button>;
        })}
      </div>
    </section>
  );
